    ...
    try:
        ...
        user_data = {
            ...
            "budget_range": (
                ...
                if data.get("budgetMin") and data.get("budgetMax")
                else ""
            ),
        }
...
def get_shopping_recommendations():
    ...
    try:
        ...
        # API key for Gemini
        api_key = "AIzaSyDnzIiI4fyTjev59G4mHvW46amYTj83uvA"

        # Get categories from Gemini
        categories = build_and_get_categories(
            api_key, user_input, user_data["user_location"], user_data
        )
            ...
            for i, ai_product in enumerate(ai_recommendations):
                ...
                if ai_title_key in scraped_products_map:
                    ...
                    for scraped_title_key, product in scraped_products_map.items():
                        if (ai_title_key in scraped_title_key or 
                            scraped_title_key in ai_title_key):
                ...
                if scraped_product:
                    ...
                    if scraped_product.get("average_rating"):
                        try:
                            ...
                            rating_clean = "".join(c for c in rating_str if c.isdigit() or c == ".")
                            if rating_clean:
